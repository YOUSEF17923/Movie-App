...
function search(){
    ...
    for(var i = 0 ; i < data.length;i++){
            if (getname(i).includes(valueOfSearch.value)){
                cartona += `
                <div class="col-4 pt-5 px-5 ">
                <div class="item overflow-hidden position-relative">
                    <figure>
...
valueOfSearch.addEventListener('keydown',function(){
       search(); 
})
...
function changeButton(){
    st_mar= document.querySelector('.button-sl').style.marginLeft;
    document.querySelector('.button-sl').classList.add('btn-danger');
    document.querySelector('.button-sl').addEventListener('mousemove',function(){
    
    if (st_mar =='10px'){
    document.querySelector('.button-sl').style.marginLeft ='300px';
    st_mar = '300px'
    }
    else if (st_mar =='300px'){
        document.querySelector('.button-sl').style.marginLeft ='10px';
         st_mar = '10px';
    }
...
function rebutton(){
        if (regixName.test(nameInputV.value) == true && regiMail.test(mailInputV.value)== true && regixPhone.test(phoneInputV.value)== true && regixAge.test(ageInputV.value)== true && regixPasswd.test(passInputV.value)== true && repassInputV.value == passInputV.value){
            {
                document.querySelector('.button-sl').classList.remove('btn-danger');
                document.querySelector('.button-sl').style.display = "none";
                document.querySelector('.button-sl2').style.display = "block";
            }
